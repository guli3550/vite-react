...
-- Run this version instead of the previous repair script.
...
-- The old database has images/sizes/colors as TEXT. Convert them safely to JSONB.
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='products' and column_name='images' and data_type='text') then
    alter table public.products add column images_json jsonb default '[]'::jsonb;
    update public.products set images_json = case when images is null or trim(images) = '' then '[]'::jsonb else '[]'::jsonb end;
    alter table public.products drop column images;
    alter table public.products rename column images_json to images;
  ...
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='products' and column_name='sizes' and data_type='text') then
    alter table public.products add column sizes_json jsonb default '[]'::jsonb;
    update public.products set sizes_json = '[]'::jsonb;
    alter table public.products drop column sizes;
    alter table public.products rename column sizes_json to sizes;
  ...
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='products' and column_name='colors' and data_type='text') then
    alter table public.products add column colors_json jsonb default '[]'::jsonb;
    update public.products set colors_json = '[]'::jsonb;
    alter table public.products drop column colors;
    alter table public.products rename column colors_json to colors;
