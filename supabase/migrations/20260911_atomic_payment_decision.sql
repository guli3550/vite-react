...
begin
  ...
  if coalesce(v_order.payment,'') <> 'card_manual' then raise exception 'Bu buyurtma manual karta to‘lovi emas'; end if;
  if v_order.status = 'Yetkazildi' then raise exception 'Yetkazilgan buyurtmaning payment decision holatini o‘zgartirib bo‘lmaydi'; end if;

  if p_payment_status = 'verified' then
    if nullif(trim(coalesce(v_order.payment_receipt_path,'')),'') is null then
    ...
    if coalesce(v_order.payment_status,'pending') not in ('pending','receipt_uploaded') then
      raise exception 'To‘lov bo‘yicha qaror allaqachon qabul qilingan';
    end if;
    if v_order.status in ('Qabul qilindi','⏳ Buyurtma kutilmoqda','⏳ To‘lovni tasdiqlash kutilmoqda') then
      v_order_status := 'Bekor qilindi';
    else
      raise exception 'Buyurtma allaqachon yetkazish jarayoniga o‘tgan; to‘lovni rad etish mumkin emas';
    end if;
  end if;
