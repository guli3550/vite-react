-- GULI checkout runtime repair: tolerate legacy UUID product IDs.
-- Run once in Supabase SQL Editor after repair_existing_schema.sql.
-- This migration is idempotent and only replaces the secure checkout RPC.
...
begin
  ...
  if order_no is not null then
    select to_jsonb(o) into existing from orders o where o.order_number = order_no and o.telegram_id::text = p_telegram_id::text limit 1;
    if existing is not null then return existing; end if;
  end if;

  if to_regclass('public.telegram_users') is not null then
    select tu.telegram_phone into telegram_phone from telegram_users tu where tu.telegram_id::text = p_telegram_id::text limit 1;
  end if;
  ...
  for it in select value from jsonb_array_elements(p_order->'items') loop
    ...
    -- Prefer the immutable six-digit product code when the client supplies it.
    -- Otherwise compare the database id as text so both UUID and bigint legacy
    -- product tables remain compatible without unsafe casts.
    product_code_key := coalesce(it->'product'->>'product_code', it->>'product_code');
    product_id_key := coalesce(it->'product'->>'id', it->>'product_id');

    if product_code_key is not null and product_code_key <> '' then
      select * into prod from products where product_code = product_code_key for update;
    elsif product_id_key is not null and product_id_key <> '' then
      select * into prod from products where id::text = product_id_key for update;
    else
    ...
    update products set stock = stock - qty, updated_at = now() where id::text = prod.id::text;
  end loop;
