...
begin
  ...
  if to_regclass('public.telegram_users') is not null then
    ...
    where tu.telegram_id::text = p_telegram_id::text
    limit 1;
  end if;
  ...
  for it in select value from jsonb_array_elements(p_order->'items') loop
    ...
    -- Never cast the client supplied product id to bigint. Legacy stores can
    -- have UUID product IDs. Comparing the database id as text supports both
    -- UUID and bigint schemas while product_code remains the preferred key.
    ...
    if product_key is null or product_key = '' then
      raise exception 'Mahsulot identifikatori topilmadi';
    end if;

    if product_key ~ '^[0-9]{6}$' then
      select * into prod
      from products
      where product_code = product_key
      for update;
    else
      select * into prod
      from products
      where id::text = product_key
      for update;
    end if;
