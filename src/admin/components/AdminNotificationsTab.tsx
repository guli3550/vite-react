...
export type BroadcastMsg = {
  id: string; title: string; body: string; imageUrl?: string; imageCount?: number;
...
async function prepareImage(file: File): Promise<SelectedImage> {
  const raw = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result || "")); reader.onerror = () => reject(new Error("Rasmni o'qib bo'lmadi")); reader.readAsDataURL(file); });
  try {
    const img = await new Promise<HTMLImageElement>((resolve, reject) => { const el = new Image(); el.onload = () => resolve(el); el.onerror = () => reject(new Error("Rasm formati qo'llab-quvvatlanmadi")); el.src = raw; });
    const maxSide = 1400; const scale = Math.min(1, maxSide / Math.max(img.naturalWidth || img.width, img.naturalHeight || img.height));
    if (scale === 1 && raw.length <= 700000) return { id: `${Date.now()}-${Math.random()}`, dataUrl: raw, name: file.name };
    ...
    const ctx = canvas.getContext("2d"); if (!ctx) return { id: `${Date.now()}-${Math.random()}`, dataUrl: raw, name: file.name };
    ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
    return { id: `${Date.now()}-${Math.random()}`, dataUrl: canvas.toDataURL("image/jpeg", 0.78), name: file.name };
  } catch { return { id: `${Date.now()}-${Math.random()}`, dataUrl: raw, name: file.name }; }
}
...
export function AdminNotificationsTab({ notify }: { notify: (m: string) => void }) {
  ...
  const resetImages = () => { setMainImage(null); setAdditionalImages([]); setAdImageUrl(""); if (mainInputRef.current) mainInputRef.current.value = ""; if (extraInputRef.current) extraInputRef.current.value = ""; };
  const openCreate = () => { setTitle(""); setBody(""); resetImages(); setButtonText("🛍️ Online Marketni Ochish"); setTarget("groups"); setLastError(""); setModalOpen(true); };

  const chooseMain = async (files: FileList | null) => {
    const file = files?.[0]; if (!file) return;
    try { setMainImage(await prepareImage(file)); } catch (e) { notify(e instanceof Error ? e.message : "Rasmni yuklab bo'lmadi"); }
  };
  const chooseAdditional = async (files: FileList | null) => {
    if (!files?.length) return;
    const slots = Math.max(0, 9 - additionalImages.length); const selected = Array.from(files).slice(0, slots);
    try { const prepared = await Promise.all(selected.map(prepareImage)); setAdditionalImages(prev => [...prev, ...prepared]); if (files.length > slots) notify("Telegram albomida maksimal 10 ta rasm bo'lishi mumkin"); } catch (e) { notify(e instanceof Error ? e.message : "Qo'shimcha rasmlarni yuklab bo'lmadi"); }
  };
  const removeAdditional = (id: string) => setAdditionalImages(prev => prev.filter(image => image.id !== id));

  const sendBroadcast = async (e: React.FormEvent) => {
    e.preventDefault();
    const cleanTitle = title.trim(), cleanBody = body.trim(), cleanChannel = telegramChannelId.trim(), cleanUrl = adImageUrl.trim();
    if (!cleanTitle && !cleanBody) return notify("Reklama matnini kiriting");
    if (!mainImage && !cleanUrl && additionalImages.length) return notify("Qo'shimcha rasmlar uchun avval asosiy rasmni tanlang");
    if (target !== "users" && !registeredGroups.length && !cleanChannel) return notify("Avval Telegram kanal/guruhni sozlang");
    const images = [mainImage?.dataUrl || cleanUrl, ...additionalImages.map(image => image.dataUrl)].filter(Boolean);
    if (images.length > 10) return notify("Maksimal 10 ta rasm yuborish mumkin");
    if (images.reduce((sum, value) => sum + value.length, 0) > 3600000) return notify("Rasmlar hajmi juda katta. Rasmlarni kichraytirib qayta tanlang.");
    setIsSending(true); setLastError("");
    try {
      const res = await adminRequest("/api/admin/broadcast-telegram", { method: "POST", body: JSON.stringify({ title: cleanTitle, body: cleanBody, imageUrl: cleanUrl, imageUrls: images, target, telegramChannelId: cleanChannel, buttonText: buttonText.trim() || "🛍️ Online Marketni Ochish", buttonUrl: MINI_APP_URL }) });
      const json: ApiResponse = await res.json().catch(() => ({}));
      if (!res.ok || json.success !== true) throw new Error(json.message || `Reklama yuborilmadi (${res.status})`);
      const sent = Number(json.data?.sent || 0), failed = Number(json.data?.failed || 0), imageCount = Number(json.data?.imageCount || images.length || 0);
      if (sent <= 0) throw new Error(json.message || "Telegram hech bir chatga reklamani yubormadi");
      const record: BroadcastMsg = { id: `BC-${Date.now()}`, title: cleanTitle, body: cleanBody, imageUrl: cleanUrl || undefined, imageCount, target, sentAt: new Date().toISOString(), recipientsCount: sent, failedCount: failed };
  ...
  const currentImageCount = (mainImage ? 1 : 0) + additionalImages.length + (!mainImage && adImageUrl.trim() ? 1 : 0);
  return <div className="dash" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
    ...
    <section className="proPanel tablePanel"><div className="panelHead"><div><span className="proEyebrow">REAL SEND HISTORY</span><h2>Yuborilgan reklamalar ({broadcasts.length})</h2></div></div>{broadcasts.length === 0 ? <p style={{ color: "#64748b" }}>Hali muvaffaqiyatli reklama yuborilmagan.</p> : <div className="tableScroll"><table><thead><tr><th>ID</th><th>Sarlavha</th><th>Auditoriya</th><th>Rasmlar</th><th>Yuborildi</th><th>Xatolik</th><th>Vaqt</th></tr></thead><tbody>{broadcasts.map((b) => <tr key={b.id}><td><b className="promoCode">{b.id.slice(-8)}</b></td><td><b>{b.title || "(Sarlavhasiz)"}</b><div className="broadcastBodyPreview">{b.body}</div></td><td>{b.target === "groups" ? "📢 Bot admin chatlar" : b.target === "users" ? "👤 Telegram mijozlar" : "🚀 Barchasi"}</td><td>{b.imageCount ? `🖼️ ${b.imageCount}` : "—"}</td><td><span className="pill bluePill">✈️ {b.recipientsCount}</span></td><td>{b.failedCount ? <span style={{ color: "#b91c1c" }}>{b.failedCount}</span> : "—"}</td><td>{new Date(b.sentAt).toLocaleString("uz-UZ", { dateStyle: "short", timeStyle: "short" })}</td></tr>)}</tbody></table></div>}</section>
    {modalOpen && <div className="modalShade" onMouseDown={() => !isSending && setModalOpen(false)}><div className="proModal" style={{ maxWidth: 650 }} onMouseDown={(e) => e.stopPropagation()}><div className="modalHead"><div><span className="proEyebrow">NEW BROADCAST</span><h2>📢 Yangi reklama yuborish</h2></div><button type="button" onClick={() => setModalOpen(false)} disabled={isSending}>×</button></div><form onSubmit={sendBroadcast}><div className="formGrid">
      <label>Maqsadli auditoriya<select value={target} onChange={(e) => setTarget(e.target.value as "groups" | "users" | "all")}><option value="groups">📢 Bot admin bo‘lgan guruhlar/kanallar ({adminGroups.length})</option><option value="users">👤 Telegram mijozlar</option><option value="all">🚀 Barchasi</option></select></label>
      <label>Reklama sarlavhasi<input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="🌸 Yangi kolleksiya!" /></label>
      <div className="fullRow" style={{ display: "flex", flexDirection: "column", gap: 8 }}><span style={{ fontSize: 12, fontWeight: 700 }}>🖼️ Reklama rasmlari <small style={{ color: "#64748b", fontWeight: 500 }}>({currentImageCount}/10)</small></span><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(110px,1fr))", gap: 8 }}>
        <button type="button" onClick={() => mainInputRef.current?.click()} disabled={isSending} style={{ minHeight: 105, border: "1.5px dashed #cbd5e1", borderRadius: 12, background: "#f8fafc", cursor: "pointer", padding: 6 }}><div style={{ fontSize: 25 }}>🖼️</div><b style={{ fontSize: 11 }}>Asosiy rasm</b><div style={{ fontSize: 10, color: "#64748b", marginTop: 3 }}>1 ta tanlang</div></button>
        {mainImage && <div style={{ position: "relative", borderRadius: 12, overflow: "hidden", border: "2px solid #c75a7a", background: "#fff" }}><img src={mainImage.dataUrl} alt="Asosiy rasm" style={{ width: "100%", height: 105, objectFit: "cover", display: "block" }} /><span style={{ position: "absolute", left: 5, bottom: 5, background: "#c75a7a", color: "#fff", borderRadius: 6, padding: "3px 6px", fontSize: 9, fontWeight: 800 }}>ASOSIY</span><button type="button" onClick={() => setMainImage(null)} style={{ position: "absolute", right: 5, top: 5, border: 0, borderRadius: "50%", width: 24, height: 24, background: "rgba(0,0,0,.65)", color: "#fff" }}>×</button></div>}
        {additionalImages.map((image, index) => <div key={image.id} style={{ position: "relative", borderRadius: 12, overflow: "hidden", border: "1px solid #e2e8f0", background: "#fff" }}><img src={image.dataUrl} alt={`Qo'shimcha ${index + 1}`} style={{ width: "100%", height: 105, objectFit: "cover", display: "block" }} /><span style={{ position: "absolute", left: 5, bottom: 5, background: "rgba(15,23,42,.75)", color: "#fff", borderRadius: 6, padding: "3px 6px", fontSize: 9 }}>#{index + 2}</span><button type="button" onClick={() => removeAdditional(image.id)} style={{ position: "absolute", right: 5, top: 5, border: 0, borderRadius: "50%", width: 24, height: 24, background: "rgba(0,0,0,.65)", color: "#fff" }}>×</button></div>)}
        {mainImage && additionalImages.length < 9 && <button type="button" onClick={() => extraInputRef.current?.click()} disabled={isSending} style={{ minHeight: 105, border: "1.5px dashed #cbd5e1", borderRadius: 12, background: "#fff", cursor: "pointer", padding: 6 }}><div style={{ fontSize: 25 }}>➕</div><b style={{ fontSize: 11 }}>Qo‘shimcha rasm</b><div style={{ fontSize: 10, color: "#64748b", marginTop: 3 }}>Bir nechta tanlash mumkin</div></button>}
      </div><input ref={mainInputRef} type="file" accept="image/*" hidden onChange={(e) => chooseMain(e.target.files)} /><input ref={extraInputRef} type="file" accept="image/*" multiple hidden onChange={(e) => chooseAdditional(e.target.files)} /><input value={adImageUrl} onChange={(e) => setAdImageUrl(e.target.value)} placeholder="URL orqali asosiy rasm (ixtiyoriy): https://..." style={{ marginTop: 2 }} /><small style={{ color: "#64748b", fontSize: 10 }}>Asosiy rasm + qo‘shimcha rasmlar Telegram albomi sifatida yuboriladi.</small></div>
      <label className="fullRow">Reklama matni<textarea rows={5} required value={body} onChange={(e) => setBody(e.target.value)} placeholder="Aksiya, chegirma yoki mahsulot haqida yozing..." /></label>
      <label className="fullRow">Tugma matni<input value={buttonText} onChange={(e) => setButtonText(e.target.value)} /></label>
    </div>{lastError && <div style={{ marginTop: 12, padding: 12, borderRadius: 10, background: "#fef2f2", border: "1px solid #fecaca", color: "#991b1b", fontSize: 13 }}>❌ {lastError}</div>}<div className="modalActions" style={{ marginTop: 18 }}><button type="button" onClick={() => setModalOpen(false)} disabled={isSending}>Bekor qilish</button><button type="submit" className="proPrimary" disabled={isSending}>{isSending ? "⏳ Telegramga yuborilmoqda..." : "🚀 Reklamani yuborish"}</button></div></form></div></div>}
  </div>;
}
