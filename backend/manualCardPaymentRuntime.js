...
async function bucket(){if(!supabase)throw new Error("Supabase sozlanmagan");const x=await supabase.storage.getBucket(BUCKET);if(!x.error)return;const y=await supabase.storage.createBucket(BUCKET,{public:false,allowedMimeTypes:["image/jpeg","image/png","image/webp","application/pdf"],fileSizeLimit:"6MB"});if(y.error&&!/already exists|duplicate/i.test(y.error.message||""))throw y.error}
...
install("post","/api/guest/orders",async(req,res)=>{
  ...
  try{
    if(!supabase)throw new Error("Supabase sozlanmagan");const {phone,items,address,payment,status,promo_code}=req.body||{};
    ...
    const order={order_number:null,username:null,first_name:null,phone:String(phone).trim(),items,address:address||null,payment:payment||"card_manual",status:status||"Qabul qilindi",promo_code:promo_code?String(promo_code).trim().toUpperCase():""};
...
install("post","/api/orders/:orderNumber/receipt",async(req,res)=>{
  ...
  try{
    ...
    const {data,mimeType,extension}=req.body||{};if(!data||typeof data!=="string")return fail(res,400,"Chek rasmi topilmadi");if(!/^image\/(jpeg|png|webp)$/.test(String(mimeType||""))&&mimeType!=="application/pdf")return fail(res,400,"Chek faqat JPG, PNG, WEBP yoki PDF bo‘lishi mumkin");if(data.length>8200000)return fail(res,413,"Chek hajmi juda katta");
    await bucket();const ext=String(extension||(mimeType==="application/pdf"?"pdf":"jpg")).replace(/[^a-z0-9]/gi,"").toLowerCase()||"jpg";const path=`receipts/${order.id}/${Date.now()}-${crypto.randomBytes(6).toString("hex")}.${ext}`;
    const {error:up}=await supabase.storage.from(BUCKET).upload(path,Buffer.from(data,"base64"),{contentType:mimeType,cacheControl:"31536000",upsert:false});if(up)throw up;
    const {data:updated,error:ue}=await supabase.from("orders").update({payment_receipt_path:path,payment_status:"receipt_uploaded",payment_receipt_uploaded_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",order.id).eq("telegram_id",user.id).select("id,order_number,total,payment_status").single();if(ue)throw ue;
    return res.json({success:true,message:"Chek muvaffaqiyatli yuborildi. Admin tekshiradi.",data:updated})
  }catch(e){console.error("Receipt upload error:",e);return res.status(/payment_receipt_path|payment_status|payment_receipt_uploaded_at/i.test(e.message||"")?503:500).json({success:false,message:/payment_receipt_path|payment_status|payment_receipt_uploaded_at/i.test(e.message||"")?"To‘lov chek ustunlari bazada hali tayyor emas. SQL migration'ni bir marta ishga tushiring.":"Chekni yuborishda xatolik"})}
});
install("get","/api/admin/orders/:id/payment-receipt",async(req,res)=>{
  ...
  try{if(!supabase)throw new Error("Supabase sozlanmagan");const {data:order,error}=await supabase.from("orders").select("id,order_number,total,payment,payment_status,payment_receipt_path,payment_receipt_uploaded_at,payment_verified_at").eq("id",req.params.id).maybeSingle();if(error)throw error;if(!order)return fail(res,404,"Buyurtma topilmadi");if(!order.payment_receipt_path)return fail(res,404,"Bu buyurtmaga chek yuborilmagan");await bucket();const {data:signed,error:se}=await supabase.storage.from(BUCKET).createSignedUrl(String(order.payment_receipt_path).replace(/^\/+/,""),900);if(se)throw se;return res.json({success:true,data:{...order,receipt_url:signed.signedUrl}})}catch(e){console.error("Admin receipt view error:",e);return res.status(500).json({success:false,message:"Chekni ochishda xatolik"})}
});
install("put","/api/admin/orders/:id/payment",async(req,res)=>{
  if(!admin(req))return fail(res,401,"Admin sessiyasi yaroqsiz yoki tugagan");
  try{if(!supabase)throw new Error("Supabase sozlanmagan");const s=["pending","receipt_uploaded","verified","rejected"].includes(String(req.body?.payment_status))?String(req.body.payment_status):null;if(!s)return fail(res,400,"To‘lov holati noto‘g‘ri");const patch={payment_status:s,payment_verified_at:s==="verified"?new Date().toISOString():null,updated_at:new Date().toISOString()};const {data,error}=await supabase.from("orders").update(patch).eq("id",req.params.id).select("*").single();if(error)throw error;return res.json({success:true,data})}catch(e){console.error("Admin payment status error:",e);return res.status(/payment_status|payment_verified_at/i.test(e.message||"")?503:500).json({success:false,message:/payment_status|payment_verified_at/i.test(e.message||"")?"To‘lov ustunlari bazada hali tayyor emas. SQL migration'ni ishga tushiring.":"To‘lov holatini yangilashda xatolik"})}
});
