...
const TOOL_ROUTES = Object.freeze({
  ...
  chat_inspect: "support",
  security_audit: "security"
});
const ALLOWED_TOOLS = new Set(Object.keys(TOOL_ROUTES));
...
function chooseRoute(text, requestedTool) {
  const explicit = String(requestedTool || "").trim();
  if (ALLOWED_TOOLS.has(explicit)) return { agentId: TOOL_ROUTES[explicit], tool: explicit, reason: "explicit_tool" };
  const value = String(text || "").toLowerCase();
  if (/to['’`]?lov|payment|humo|uzcard|chek|receipt/.test(value)) return { agentId: "payment", tool: "payment_status", reason: "payment_keywords" };
  if (/buyurtma|order|zakaz|status|yetkaz/.test(value)) return { agentId: "order", tool: "order_lookup", reason: "order_keywords" };
  if (/chat|mijoz|telegram|xabar|suhbat/.test(value)) return { agentId: "support", tool: "chat_inspect", reason: "support_keywords" };
  if (/xavfsiz|security|audit|risk|token|rls/.test(value)) return { agentId: "security", tool: "security_audit", reason: "security_keywords" };
  return { agentId: "sales", tool: "catalog_check", reason: "default_catalog_route" };
}

install("post", "/api/admin/agents/dispatch", requireAgentAdmin, async (req, res) => {
  try {
    const taskText = String(req.body?.task || "").trim();
    const requestedTool = String(req.body?.tool || "").trim();
    if (!taskText && !requestedTool) return res.status(400).json({ success: false, message: "Task tavsifi yoki tool kerak" });
    if (taskText.length > 1000) return res.status(400).json({ success: false, message: "Task tavsifi juda uzun" });
    const route = chooseRoute(taskText, requestedTool);
    const input = req.body?.input && typeof req.body.input === "object" && !Array.isArray(req.body.input) ? req.body.input : {};
    const command = JSON.stringify({ type: route.tool, input });
    const { data, error } = await supabase.from("agent_tasks").insert({ agent_id: route.agentId, command, status: "queued", created_by: "orchestrator", created_at: new Date().toISOString() }).select("*").single();
    if (error) throw error;
    await emitEvent({ taskId: data.id, agentId: route.agentId, eventType: "task_created", message: `Orchestrator taskni ${route.agentId} agentga taqsimladi`, metadata: { route_reason: route.reason, tool: route.tool, requested_tool: requestedTool || null }, createdBy: "orchestrator" });
    res.status(201).json({ success: true, data, routing: route });
  } catch (error) { console.error("Agent dispatch error:", error); res.status(500).json({ success: false, message: "Orchestrator taskni taqsimlay olmadi" }); }
});
...
install("post", "/api/admin/agents/tasks/:id/pause", requireAgentAdmin, async (req, res) => {
  try {
    const { data, error } = await supabase.from("agent_tasks").update({ status: "paused" }).eq("id", req.params.id).eq("status", "queued").select("*").maybeSingle();
    if (error) throw error;
    if (!data) return res.status(409).json({ success: false, message: "Faqat navbatdagi task pauzalanadi; ishlayotgan taskni majburan to'xtatish xavfsiz emas" });
    await emitEvent({ taskId: data.id, agentId: data.agent_id, eventType: "task_cancelled", message: "Agent task paused by admin", metadata: { control: "pause" }, createdBy: "admin" });
    res.json({ success: true, data });
  } catch (error) { console.error("Agent task pause error:", error); res.status(500).json({ success: false, message: "Task pauza qilinmadi" }); }
});

install("post", "/api/admin/agents/tasks/:id/resume", requireAgentAdmin, async (req, res) => {
  try {
    const { data, error } = await supabase.from("agent_tasks").update({ status: "queued", finished_at: null, error: null }).eq("id", req.params.id).eq("status", "paused").select("*").maybeSingle();
    if (error) throw error;
    if (!data) return res.status(409).json({ success: false, message: "Pauzadagi task topilmadi" });
    await emitEvent({ taskId: data.id, agentId: data.agent_id, eventType: "task_retried", message: "Agent task resumed by admin", metadata: { control: "resume" }, createdBy: "admin" });
    res.json({ success: true, data });
  } catch (error) { console.error("Agent task resume error:", error); res.status(500).json({ success: false, message: "Task qayta ishga tushirilmadi" }); }
});

install("post", "/api/admin/agents/tasks/:id/stop", requireAgentAdmin, async (req, res) => {
  try {
    const { data, error } = await supabase.from("agent_tasks").update({ status: "cancelled", finished_at: new Date().toISOString() }).eq("id", req.params.id).in("status", ["queued", "running", "paused"]).select("*").maybeSingle();
    if (error) throw error;
    ...
    res.json({ success: true, data });
  } catch (error) { console.error("Agent task stop error:", error); res.status(500).json({ success: false, message: "Agent task to'xtatilmadi" }); }
});
