...
async function sendProduct(chatId, product) {
  ...
  const markup = replyMarkup(product);
  if (image) {
    await telegramApi("sendPhoto", { chat_id: chatId, photo: image, caption: caption(product), parse_mode: "HTML", reply_markup: markup });
  } else {
    await telegramApi("sendMessage", { chat_id: chatId, text: caption(product), parse_mode: "HTML", reply_markup: markup, disable_web_page_preview: true });
  }
  return true;
}
...
async function publishProduct(product) {
  ...
  if (!targets.length) {
    console.warn("[Telegram product] target chatlar sozlanmagan; TELEGRAM_PRODUCT_CHAT_IDS ni kiriting yoki telegram_users jadvalini to‘ldiring.");
    return;
  }
  let ok = 0;
  for (const chatId of targets) {
    try {
      if (await sendProduct(chatId, product)) ok++;
    } catch (error) {
      console.warn(`[Telegram product] ${chatId} yuborilmadi:`, error.message);
    }
