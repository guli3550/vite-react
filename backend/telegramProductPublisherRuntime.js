...
async function sendProduct(chatId, product) { const image = String(product?.image || "").trim(); if (!image) return false; await telegramApi("sendPhoto", { chat_id: chatId, photo: image, caption: caption(product), parse_mode: "HTML", reply_markup: { inline_keyboard: [[{ text: "🛍️ Online Market", url: homeUrl() }, { text: "🤩 Sotib olish", url: productUrl(product) }]] } }); return true; }
