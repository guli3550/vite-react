...
async function sendProduct(chatId, product) { const image = String(product?.image || "").trim(); if (!image) return false; const exactProductUrl = productUrl(product); await telegramApi("sendPhoto", { chat_id: chatId, photo: image, caption: caption(product), parse_mode: "HTML", reply_markup: { inline_keyboard: [[{ text: "🛍️ Online Market", url: homeUrl() }, { text: "🤩 Sotib olish", url: exactProductUrl }], [{ text: "🌐 Browser + Chat", login_url: { url: exactProductUrl, bot_username: "guli3550bot", request_write_access: false } }]] } }); return true; }
...
async function publishProduct(product) { if (!BOT_TOKEN || !product || product.active === false) return; const targets = [...new Set([...PRODUCT_CHAT_IDS, ...(await getUserChatIds())])]; if (!targets.length) { console.warn("[Telegram product] target chatlar sozlanmagan; TELEGRAM_PRODUCT_CHAT_IDS ni kiriting."); return; } let ok = 0; for (const chatId of targets) { try { if (await sendProduct(chatId, product)) ok++; } catch (error) { console.warn(`[Telegram product] ${chatId} yuborilmadi:`, error.message); } await new Promise(resolve => setTimeout(resolve, 40)); } console.log(`[Telegram product] ${product.product_code || product.id || "?"}: ${ok}/${targets.length} chatga yuborildi`); }
