...
async function sendOrEditOrder(order) {
  ...
  if (existingId) {
    try {
      ...
      console.warn("[Telegram customer bot] In-place edit failed, keeping single message:", err.message);
      // Do NOT send a second message if it's already sent once!
      return;
    }
  }
  try {
...
async function configureTelegram() { if (!TELEGRAM_BOT_TOKEN) return console.warn("[Telegram] TELEGRAM_BOT_TOKEN sozlanmagan"); try { await telegramApi("setMyCommands", { commands: [{ command: "start", description: "GULI do‘konini ochish" }, { command: "shop", description: "Onlayn do‘konni ochish" }] }); await telegramApi("setChatMenuButton", { menu_button: { type: "web_app", text: STORE_TEXT, web_app: { url: WEB_APP_URL } } }); const webhookBase = process.env.RENDER_EXTERNAL_URL || "https://guli-lingerie-api.onrender.com"; await telegramApi("setWebhook", { url: `${webhookBase}/api/telegram/webhook`, allowed_updates: ["message", "edited_message", "channel_post", "edited_channel_post", "callback_query", "my_chat_member", "chat_member"] }); console.log(`[Telegram] menu + commands + webhook configured: ${WEB_APP_URL}`); } catch (error) { console.error("[Telegram] configuration failed:", error.message); } }
...
function wrapWebhook() { const original = express.application.post; express.application.post = function patchedPost(routePath, ...handlers) { if (routePath === "/api/telegram/webhook" && handlers.length) { const index = handlers.length - 1; const handler = handlers[index]; handlers[index] = async function telegramWebhookHandler(req, res, next) { const message = req.body?.message; const chatId = Number(message?.chat?.id || 0); const text = String(message?.text || "").trim(); const contact = message?.contact; try { if (chatId && /^\/start(?:@\w+)?/i.test(text)) { await telegramApi("sendMessage", { chat_id: chatId, text: "🌷 <b>GULI_3550 Online Market</b> ga xush kelibsiz!\n\nAyollar uchun ichki kiyimlar, komplektlar, uy kiyimlari va uy kiyimlarini onlayn buyurtma qilishingiz mumkin.\n\n📦 Mahsulot tanlang → buyurtma bering → HUMO/UZCARD orqali to‘lang → chekni shu oynadan yuboring.\n\nAvval telefon raqamingizni yuboring, keyin Telegram menyusidagi <b>Do‘konni ochish</b> tugmasidan foydalaning.", parse_mode: "HTML", reply_markup: { keyboard: [[{ text: "📱 Telefon raqamimni yuborish", request_contact: true }]], resize_keyboard: true, one_time_keyboard: true } }); return res.sendStatus(200); } if (chatId && /^\/(shop|store)(?:@\w+)?/i.test(text)) { await telegramApi("sendMessage", { chat_id: chatId, text: "🛍 Do‘kon Telegram menyusidagi tugma orqali ochiladi.", reply_markup: storeMenuKeyboard() }); return res.sendStatus(200); } if (chatId && contact?.phone_number) { const from = message?.from || {}; const telegramId = Number(contact.user_id || from.id || chatId); if (telegramId && supabase) await supabase.from("telegram_users").upsert({ telegram_id: telegramId, username: from.username || null, first_name: from.first_name || null, last_name: from.last_name || null, telegram_phone: String(contact.phone_number), updated_at: new Date().toISOString() }, { onConflict: "telegram_id" }); await telegramApi("sendMessage", { chat_id: chatId, text: "✅ Telefon raqamingiz saqlandi. Endi Telegram yuqorisidagi <b>Do‘konni ochish</b> menyu tugmasidan kiring.", parse_mode: "HTML", reply_markup: storeMenuKeyboard() }); return res.sendStatus(200); } return await handler(req, res, next); } catch (error) { console.error("[Telegram] webhook handler error:", error.message); return res.sendStatus(200); } }; } return original.call(this, routePath, ...handlers); }; }
wrapWebhook();
