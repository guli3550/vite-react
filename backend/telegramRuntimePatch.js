...
// This patch is intentionally Express-prototype based so it works with the
// existing backend without requiring index.js to export app/supabase/token.
...
function storeKeyboard(inline = false) {
  if (inline) return { inline_keyboard: [[{ text: STORE_TEXT, web_app: { url: WEB_APP_URL } }]] };
  return { keyboard: [[{ text: STORE_TEXT, web_app: { url: WEB_APP_URL } }]], resize_keyboard: true, is_persistent: true };
}
...
async function sendOrEditOrder(order) {
  ...
  try {
    ...
    if (messageId && supabase) {
      await supabase.from("orders").update({ telegram_status_message_id: messageId }).eq("order_number", String(fullOrder.order_number));
    }
  } catch (error) {
...
async function configureTelegram() {
  if (!TELEGRAM_BOT_TOKEN) {
    console.warn("[Telegram] TELEGRAM_BOT_TOKEN sozlanmagan");
    return;
  }
  try {
    ...
    await telegramApi("setChatMenuButton", {
      menu_button: { type: "web_app", text: STORE_TEXT, web_app: { url: WEB_APP_URL } },
    });
    ...
    if (routePath === path && handlers.length) {
      ...
      handlers[index] = async function telegramWrappedHandler(req, res, next) {
        ...
        try {
          ...
          try { await after(req, payload); } catch (error) { console.warn(`[Telegram] ${path} notification failed:`, error.message); }
        }
      };
    }
      ...
      handlers[index] = async function telegramWebhookHandler(req, res, next) {
        ...
        try {
          ...
          if (chatId && /^\/(shop|store)(?:@\w+)?/i.test(text)) {
            await telegramApi("sendMessage", { chat_id: chatId, text: "🛍 GULI PREMIUM do‘konini oching:", reply_markup: storeKeyboard(false) });
            return res.sendStatus(200);
          }
          if (chatId && contact?.phone_number) {
            ...
            if (telegramId && supabase) {
              await supabase.from("telegram_users").upsert({
                ...
                telegram_phone: String(contact.phone_number),
                updated_at: new Date().toISOString(),
              }, { onConflict: "telegram_id" });
            }
            await telegramApi("sendMessage", {
              chat_id: chatId,
              text: "✅ Telefon raqamingiz saqlandi. Endi GULI do‘konini ochishingiz mumkin.",
              reply_markup: storeKeyboard(false),
            });
            return res.sendStatus(200);
          }
          return await handler(req, res, next);
...
wrapWebhook();
wrapRoute("post", "/api/orders", async (_req, payload) => {
  const order = Array.isArray(payload?.data) ? payload.data[0] : payload?.data;
  if (payload?.success && order) await sendOrEditOrder(order);
});
wrapRoute("post", "/api/guest/orders", async (_req, payload) => {
  const order = Array.isArray(payload?.data) ? payload.data[0] : payload?.data;
  if (payload?.success && order) await sendOrEditOrder(order);
});
wrapRoute("put", "/api/admin/orders/:id", async (_req, payload) => {
  if (payload?.success && payload?.data) await sendOrEditOrder(payload.data);
});
wrapRoute("put", "/api/admin/orders/:id/payment", async (_req, payload) => {
  if (payload?.success && payload?.data) await sendOrEditOrder(payload.data);
});
