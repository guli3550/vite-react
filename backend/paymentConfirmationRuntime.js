...
install('post','/api/orders/:orderNumber/payment-confirm',async(req,res)=>{const user=customer(req);if(!user)return fail(res,401,'Mijoz sessiyasi topilmadi.');try{let order=await getOrder(String(req.params.orderNumber||'').trim(),user.id);if(!order)return fail(res,404,'Buyurtma topilmadi');order=await expireIfNeeded(order);if(String(order.status||'')==='To‘lov qilinmadi')return fail(res,410,'10 daqiqalik to‘lov vaqti tugagan. Buyurtma “To‘lov qilinmadi” holatiga o‘tdi.');if(String(order.payment||'')!=='card_manual')return fail(res,400,'Bu buyurtma karta orqali to‘lov uchun emas.');if(!order.payment_receipt_path)return fail(res,400,'Avval to‘lov chekini yuboring.');const {data,error}=await supabase.from('orders').update({payment_status:'receipt_uploaded',updated_at:new Date().toISOString()}).eq('id',order.id).eq('telegram_id',user.id).select('*').single();if(error)throw error;return res.json({success:true,message:'To‘lov qildim qayd etildi. Admin tasdiqlashi kutilmoqda.',data:{order_number:data.order_number,status:data.status,payment_status:data.payment_status}})}catch(e){console.error('Payment confirm error:',e);return res.status(500).json({success:false,message:e.message||'To‘lovni tasdiqlashda xatolik'})}});
