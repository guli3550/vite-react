...
function chatTokenValid(req, id) {
  ...
  if (lp.length === 4 && lp[0] === "linked" && Number(lp[1]) === n && Number(lp[2]) > Date.now()) {
    const expected = crypto.createHmac("sha256", secret).update(`linked.${lp[1]}.${lp[2]}`).digest("base64url");
  ...
  if (gp.length === 3 && Number(gp[0]) === n && Number(gp[1]) > Date.now()) {
    const expected = crypto.createHmac("sha256", secret).update(`${gp[0]}.${gp[1]}`).digest("base64url");
    if (safeEqual(gp[2], expected)) return true;
  }
  if (gp.length === 2) {
    try {
      const expected = crypto.createHmac("sha256", secret).update(gp[0]).digest("base64url");
      const p = JSON.parse(Buffer.from(gp[0], "base64url").toString("utf8"));
      if (safeEqual(gp[1], expected) && (Number(p.guestId) === n || !n) && Number(p.exp) > Date.now()) return true;
    } catch {}
  }
  return false;
}
  ...
  try {
    ...
    if (!Number.isSafeInteger(id)) {
      ...
      if (gp.length === 2) {
        try {
          const p = JSON.parse(Buffer.from(gp[0], "base64url").toString("utf8"));
          if (Number.isSafeInteger(Number(p.guestId))) id = Number(p.guestId);
        } catch {}
      } else if (gp.length === 3 && Number.isSafeInteger(Number(gp[0]))) {
        id = Number(gp[0]);
      }
      if (!Number.isSafeInteger(id)) {
        const tg = telegramUser(String(req.headers["x-telegram-init-data"] || ""));
