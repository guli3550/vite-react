...
const clickPatchPath = path.join(__dirname, "clickPaymentPatch.js");
let source = fs.readFileSync(indexPath, "utf8");
const patch = fs.readFileSync(identityPatchPath, "utf8") + "\n" + fs.readFileSync(reviewPatchPath, "utf8");
const clickPatch = fs.readFileSync(clickPatchPath, "utf8");
...
// Click uses form-encoded callbacks. Add its parser before the injected routes.
const orderMarker = '\napp.post("/api/orders", requireTelegramUser, async (req, res) => {';
if (!source.includes(orderMarker)) throw new Error("customerServer: /api/orders marker not found");
source = source.replace(orderMarker, `\napp.use(express.urlencoded({ extended: false }));\n\n${clickPatch}\n${orderMarker}`);
