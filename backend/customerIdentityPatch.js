...
async function syncTelegramProfilePhotos(user) {
  ...
  for (let page = 0; page < 20; page++) {
    ...
    for (const sizes of photos) {
      ...
      if (!stored) {
        ...
        stored = { file_unique_id: largest.file_unique_id, path, width: largest.width || 0, height: largest.height || 0 };
      }
  ...
  const profilePhotos = all.length ? all : existing;
...
app.post("/api/profile/sync", requireTelegramUser, async (req, res) => {
  try {
    const photos = await syncTelegramProfilePhotos(req.telegramUser);
    res.json({ success: true, data: { photoCount: photos.length } });
  } catch (error) {
