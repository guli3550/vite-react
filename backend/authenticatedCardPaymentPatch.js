...
function stableBridgeId(userId) {
  ...
  return -(100000000000 + (Number.parseInt(hex, 16) % 899999999999));
}
...
install("post", "/api/auth/orders", async (req, res) => {
  ...
  try {
    ...
    const order = { order_number: null, username: user.user_metadata?.username || user.email?.split("@")[0] || null, first_name: user.user_metadata?.full_name || user.user_metadata?.name || null, phone: String(phone).trim(), items, address: address || null, payment: "card_manual", status: "⏳ Buyurtma kutilmoqda", promo_code: promo_code ? String(promo_code).trim().toUpperCase() : "" };
