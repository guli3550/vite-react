// Security layer loaded before backend/index.js. It wraps the public order
// route so the client cannot forge product prices, subtotal, discount, or total.
...
async function calculateOrder(body) {
  ...
  const normalizedItems = body.items.map((item) => {
    ...
    return {
      ...
      product: {
        ...
        id: product.id,
        product_code: product.product_code || code || null,
        name: product.name || product.title || item.product?.name || "Mahsulot",
        price: unitPrice,
        old_price: product.old_price ?? item.product?.old_price ?? null,
        stock: product.stock
      }
    };
  });
  ...
  if (promoCode) {
    ...
    discount = data.discount_type === "percent"
      ? Math.min(subtotal, Math.round(subtotal * Number(data.discount_value || 0) / 100))
      : Math.min(subtotal, Math.max(0, Number(data.discount_value || 0)));
    promo = data;
  }
...
async function wrappedOrderHandler(req, res, next) {
  try {
    ...
    req.body = {
      ...
      promo_code: calculated.promo?.code || null
    };
    return this.__guliOriginalOrderHandler(req, res, next);
  } catch (error) {
    console.error("Server-side order pricing validation error:", error);
    return res.status(error.status || 500).json({ success: false, message: error.message || "Buyurtma summasini tekshirishda xatolik" });
  }
}
