...
function assertSecret() { if (SECRET.length < 32) throw new Error("Server auth secret must be configured with at least 32 characters"); }
function issueAccessToken(user) {
  assertSecret();
  const now = Math.floor(Date.now() / 1000);
  const payload = { sub: String(user.id), phone: user.phone_number || user.phone || null, telegram_id: user.telegram_id != null ? Number(user.telegram_id) : null, role: "customer", iat: now, exp: now + ACCESS_TTL_SEC, jti: crypto.randomUUID() };
...
function verifyAccessToken(token) {
  ...
  try {
    ...
    if (h.alg !== "HS256" || h.typ !== "JWT" || !p.sub || p.role !== "customer" || Number(p.exp) <= Math.floor(Date.now() / 1000)) return null;
    return p;
  } catch { return null; }
}
function hashRefreshToken(token) { assertSecret(); return crypto.createHmac("sha256", SECRET).update(String(token || "")).digest("hex"); }
async function issueRefreshToken(supabase, userId) {
  assertSecret();
  const token = crypto.randomBytes(48).toString("base64url");
  const { error } = await supabase.from("auth_refresh_tokens").insert({ user_id: String(userId), token_hash: hashRefreshToken(token), expires_at: new Date(Date.now() + REFRESH_TTL_MS).toISOString() });
...
module.exports = { issueAccessToken, verifyAccessToken, issueRefreshToken, rotateRefreshToken };
