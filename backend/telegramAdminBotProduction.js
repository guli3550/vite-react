...
function customerBody(o){const items=Array.isArray(o.items)?o.items:[],lines=items.slice(0,15).map(x=>{const p=x?.product||x?.product_data||x?.productDetails||x||{};return`• ${p.name||p.title||x.name||'Mahsulot'}${p.product_code||x.product_code?` #${p.product_code||x.product_code}`:''} × ${Number(x.quantity||x.qty||1)} dona`}),ps=String(o.payment_status||'pending').toLowerCase(),pay=ps==='verified'?'✅ To‘lov tasdiqlandi':ps==='rejected'?'❌ Chek rad etildi':ps==='receipt_uploaded'?'🧾 Chek yuklangan — tekshirilmoqda':'⏳ To‘lov kutilmoqda';return[`🛍 <b>Guli Market — BUYURTMA № ${o.order_number||o.id}</b>`,'',`📦 <b>Buyurtma statusi:</b> <b>${o.status||'⏳ Buyurtma kutilmoqda'}</b>`,`💳 <b>To‘lov holati:</b> ${pay}`,'',`👗 <b>Mahsulotlar (${items.length} ta):</b>`,lines.join('\n')||'—','',`💰 <b>Jami:</b> ${money(o.total)}`].join('\n').slice(0,1024)}
async function customerMediaUrls(o){return mediaUrls(o)}
async function sendCustomerAlbum(o){const urls=await customerMediaUrls(o);if(!urls.length)return null;const media=urls.map((u,i)=>({type:'photo',media:u,...(i===0?{caption:customerBody(o),parse_mode:'HTML'}:{})}));try{const sent=await tg('sendMediaGroup',{chat_id:Number(o.telegram_id),media},CUSTOMER_BOT);const mid=Number(sent?.[0]?.message_id||0)||null;if(mid&&db)await db.from('orders').update({telegram_status_message_id:mid}).eq('id',o.id);return mid}catch(e){console.warn('[GULI customer album]',e.message);return null}}
async function deleteCustomerMessage(chatId,mid){if(!mid)return;await tg('deleteMessage',{chat_id:Number(chatId),message_id:Number(mid)},CUSTOMER_BOT).catch(()=>{})}
async function ensureCustomerStatusMessage(o){if(!CUSTOMER_BOT||!db||!o?.telegram_id)return null;const existing=Number(o.telegram_status_message_id||0)||null;if(existing)return existing;try{if(o.payment_receipt_path){return await sendCustomerAlbum(o)}const sent=await tg('sendMessage',{chat_id:Number(o.telegram_id),text:customerBody(o),parse_mode:'HTML',disable_web_page_preview:true},CUSTOMER_BOT);const mid=Number(sent?.message_id||0)||null;if(mid)await db.from('orders').update({telegram_status_message_id:mid}).eq('id',o.id);return mid}catch(e){console.warn('[GULI customer status create]',e.message);return null}}
async function updateCustomer(o){if(!CUSTOMER_BOT||!o?.telegram_id)return;let mid=Number(o.telegram_status_message_id||0)||null;if(!mid)mid=await ensureCustomerStatusMessage(o);if(!mid)return;if(o.payment_receipt_path){try{await tg('editMessageCaption',{chat_id:Number(o.telegram_id),message_id:mid,caption:customerBody(o),parse_mode:'HTML'},CUSTOMER_BOT);return}catch(e){if(!/caption|not modified|message to edit/i.test(e.message||''))console.warn('[GULI customer album edit]',e.message);await deleteCustomerMessage(o.telegram_id,mid);const next=await sendCustomerAlbum(o);if(next)return}}else{try{await tg('editMessageText',{chat_id:Number(o.telegram_id),message_id:mid,text:customerBody(o),parse_mode:'HTML',disable_web_page_preview:true},CUSTOMER_BOT);return}catch(e){if(!/not modified/i.test(e.message||''))console.warn('[GULI customer status edit]',e.message)}}}
...
async function callback(c){const chat=Number(c?.message?.chat?.id||0),from=Number(c?.from?.id||0),d=String(c?.data||'');if(!chat||chat!==from)return;const ids=await admins();if(!ids.includes(String(chat))){await tg('answerCallbackQuery',{callback_query_id:c.id,text:'⛔ Ruxsat yo‘q',show_alert:true}).catch(()=>{});return}
  ...
  if(d==='guli_ai_clear_context'){
    state.aiHistory.delete(String(chat));
    await tg('answerCallbackQuery',{callback_query_id:c.id,text:'🧠 Suhbat konteksti tozalandi'});
    await sendTrackedAiMessage(chat,'🧠 Suhbat konteksti tozalandi.');
    return;
  }
  if(d==='guli_ai_delete_messages'){
    const n=await deleteAiMessages(chat);
    await tg('answerCallbackQuery',{callback_query_id:c.id,text:'🗑 AI xabarlari o‘chirildi'});
    await tg('sendMessage',{chat_id:chat,text:`🗑️ GULI AI yuborgan ${n} ta xabar o‘chirildi.`});
    return;
  }
...
async function handleAdminAiMessage(m,ids){
  ...
  if(/^\/ai_clear(?:@\w+)?(?:\s+.*)?$/i.test(text)){
    await sendTrackedAiMessage(chat,'🧹 GULI AI — qaysi amalni bajarish kerak?',{reply_markup:{inline_keyboard:[
      [{text:'🧠 Suhbat kontekstini tozalash',callback_data:'guli_ai_clear_context'}],
      [{text:'🗑️ AI xabarlarini tozalash',callback_data:'guli_ai_delete_messages'}]
    ]}});
    return true;
  }
