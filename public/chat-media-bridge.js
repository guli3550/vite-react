...
(function () {
  ...
  function liftMedia(message) {
    const m = { ...message }; const meta = m.metadata || {};
  ...
  function liftStoredMedia() {
    const source = read(); let changed = false; const out = source.map(item => { const next = liftMedia(item); if (JSON.stringify(next) !== JSON.stringify(item)) changed = true; return next; }); if (changed) write(out);
  }
  async function uploadAttachment(id, item) {
    if (!item?.mediaUrl || !String(item.mediaUrl).startsWith("data:")) return null;
    const comma = String(item.mediaUrl).indexOf(","); if (comma < 0) return null;
    const header = String(item.mediaUrl).slice(0, comma); const data = String(item.mediaUrl).slice(comma + 1);
    const mimeType = (header.match(/^data:([^;]+);base64$/i) || [])[1] || "application/octet-stream";
    const res = await baseFetch(`${API}/api/chat/media-upload`, { method: "POST", headers: { "Content-Type": "application/json", ...authHeaders() }, body: JSON.stringify({ telegram_id: id, data, mimeType, fileName: item.fileName || "file", type: item.type || "file" }) });
    const json = await res.json().catch(() => null); if (!res.ok || !json?.success || !json?.data?.mediaUrl) throw new Error(json?.message || `media upload ${res.status}`); return json.data;
  }
  function findLocalAttachment(body) {
    ...
    const isGuest = Number(id) < 0;
    const candidates = items.filter(m => {
      const userMatches = isGuest ? String(m?.userId || "") === "guest-user" : String(m?.userId || "") === id;
    ...
    try {
      ...
      if (originalBody && !originalBody.mediaUrl) {
        ...
        if (item && String(item.mediaUrl).startsWith("data:")) {
          ...
          originalBody = { ...originalBody, mediaUrl: uploaded.mediaUrl, mediaPath: uploaded.mediaPath, fileName: uploaded.fileName || item.fileName, type: uploaded.type || item.type, mimeType: uploaded.mimeType };
          const updated = read().map(m => String(m?.id) === String(item.id) ? { ...m, mediaUrl: uploaded.mediaUrl, mediaPath: uploaded.mediaPath, fileName: uploaded.fileName || item.fileName, type: uploaded.type || item.type } : m); write(updated);
        }
        if (originalBody) init = { ...(init || {}), body: JSON.stringify(originalBody) };
      }
    } catch (e) { console.error("[GULI chat media] attachment upload failed:", e); throw e; }
    return baseFetch(input, init);
  };
  function normalizeEventMessages() { const source = read(); let changed = false; const out = source.map(item => { const next = liftMedia(item); if (JSON.stringify(next) !== JSON.stringify(item)) changed = true; return next; }); if (changed) write(out); }
  window.addEventListener("guli_chat_updated", () => setTimeout(normalizeEventMessages, 0));
  liftStoredMedia(); setTimeout(liftStoredMedia, 1000);
