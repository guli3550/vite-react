...
module.exports = async function handler(req, res) {
  ...
  try {
    const r = await fetch(`${API}/api/products/${encodeURIComponent(ref)}`, {
      headers: { Accept: "application/json" },
    });
    if (!r.ok) return res.status(r.status === 404 ? 404 : 502).send(r.status === 404 ? "Mahsulot topilmadi" : "Mahsulot xizmati vaqtincha ishlamayapti");
    ...
    const images = [p.image, ...(Array.isArray(p.images) ? p.images : [])]
      .map(absolute).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).slice(0,10);
    ...
    const schema = {
      "@context":"https://schema.org","@type":"Product",name,description,image:images,
      sku:p.product_code || p.id,category,
      brand:{"@type":"Brand",name:"Guli Market"},
      offers:{
        "@type":"Offer",url:productUrl,priceCurrency:"UZS",
        ...(price > 0 ? {price} : {}),
