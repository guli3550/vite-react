(() => {
  ...
  function openTelegram(session){
    const httpsUrl=String(session.telegram_url||'');
    try{
      const u=new URL(httpsUrl);
      const username=u.pathname.replace(/^\/+/, '').split('/')[0];
      const start=u.searchParams.get('start');
      if(username&&start){
        const tgUrl=`tg://resolve?domain=${encodeURIComponent(username)}&start=${encodeURIComponent(start)}`;
        let left=true;
        const onHide=()=>{left=false;document.removeEventListener('visibilitychange',onHide)};
        document.addEventListener('visibilitychange',onHide);
        window.location.href=tgUrl;
        setTimeout(()=>{document.removeEventListener('visibilitychange',onHide);if(left)window.open(httpsUrl,'_blank','noopener,noreferrer')},1200);
        return;
      }
    }catch(_){}
  ...
  function mount(){
    ...
    const root=document.createElement('div');root.id='guli-canonical-auth';root.innerHTML='<div class="card"><div style="width:64px;height:64px;margin:0 auto 14px auto;display:flex;align-items:center;justify-content:center;border-radius:50%;box-shadow:0 8px 24px rgba(34,158,217,0.38)"><svg width="64" height="64" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="120" cy="120" r="120" fill="url(#tg_can_grad)"/><path d="M183 67L47.5 119.3C38.3 123 38.4 128.2 45.8 130.5L80.6 141.3L161.1 90.6C164.9 88.3 168.4 89.5 165.5 92.1L100.3 151L98.9 171.7C100.9 171.7 101.8 170.8 102.9 169.7L122.1 151.2L162 180.7C169.4 184.8 174.7 182.7 176.5 173.9L202.7 50.4C205.4 39.6 198.6 34.7 183 67Z" fill="white"/><defs><linearGradient id="tg_can_grad" x1="120" y1="0" x2="120" y2="240" gradientUnits="userSpaceOnUse"><stop stopColor="#2AABEE"/><stop offset="1" stopColor="#229ED9"/></linearGradient></defs></svg></div><h2>GULI hisobiga kirish</h2><p>Telefon raqamingizni Telegram orqali tasdiqlang.<br>OTP kod kiritish shart emas.</p><button class="tg" style="display:flex;align-items:center;justify-content:center;gap:8px"><svg width="22" height="22" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M183 67L47.5 119.3C38.3 123 38.4 128.2 45.8 130.5L80.6 141.3L161.1 90.6C164.9 88.3 168.4 89.5 165.5 92.1L100.3 151L98.9 171.7C100.9 171.7 101.8 170.8 102.9 169.7L122.1 151.2L162 180.7C169.4 184.8 174.7 182.7 176.5 173.9L202.7 50.4C205.4 39.6 198.6 34.7 183 67Z" fill="white"/></svg><span>Telegram orqali kirish</span></button><div class="status">Telegram ochilgach, <b>Start</b> tugmasini bosing. Keyin <b>Telefon raqamimni yuborish</b> tugmasini bosing. Brauzer avtomatik kiradi.</div><button class="close">Yopish</button></div>';document.body.appendChild(root);
    ...
    tg.addEventListener('click',async()=>{tg.disabled=true;tg.textContent='⏳ Telegram ochilmoqda...';try{const session=await api('/api/v1/auth/init-session',{method:'POST',body:'{}'});openTelegram(session);status.textContent='📲 Telegram ochildi. Avval <b>Start</b>, keyin <b>Telefon raqamimni yuborish</b> tugmasini bosing...';let elapsed=0;timer=window.setInterval(async()=>{if(stopped)return;elapsed+=1200;if(elapsed>300000){clearInterval(timer);status.className='status error';status.textContent='Sessiya muddati tugadi. Qaytadan urinib ko‘ring.';tg.disabled=false;tg.textContent='📱 Telegram orqali kirish';return}try{const state=await api(`/api/v1/auth/check-status/${session.session_id}`);if(state.status==='EXPIRED'){clearInterval(timer);status.className='status error';status.textContent='Sessiya muddati tugadi. Qaytadan urinib ko‘ring.';tg.disabled=false;tg.textContent='📱 Telegram orqali kirish';return}if(state.status==='READY'){status.textContent='🔐 Telegram tasdig‘i olindi. Hisobingizga xavfsiz kirilmoqda...';try{const exchanged=await api('/api/v1/auth/exchange',{method:'POST',body:JSON.stringify({session_id:session.session_id,exchange_ticket:session.exchange_ticket})});clearInterval(timer);finish(exchanged)}catch(e){if(/allaqachon ishlatilgan/i.test(e.message)){clearInterval(timer);status.className='status error';status.textContent='Sessiya allaqachon ishlatilgan. Qaytadan login qiling.'}}}else if(state.status==='VERIFIED'){clearInterval(timer);status.className='status success';status.textContent='✅ Muvaffaqiyatli kirdingiz! GULI hisobingiz ochilmoqda...';setTimeout(()=>window.location.reload(),450)}else status.textContent='📲 Telegramda <b>Start</b> va keyin <b>Telefon raqamimni yuborish</b> tugmasini bosing...'}catch(_){}},1200)}catch(e){status.className='status error';status.textContent=`❌ ${e.message||'Telegram loginni boshlash imkoni bo‘lmadi.'}`;tg.disabled=false;tg.textContent='📱 Telegram orqali kirish'}});
  }
